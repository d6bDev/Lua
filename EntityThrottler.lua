...
- Hopefully fixed any and all exceptions when cleanup kicks in, caused by attempting to delete nonexistant entities due to a delayed check
...
local function add_debug_text(str, dur)
    util.create_thread(function()
        local time = util.current_time_millis() + (dur or 1000)
        while time > util.current_time_millis() do
